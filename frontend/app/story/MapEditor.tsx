...
import { TOWN_FLOOR_TEXTURES, TOWN_MAP, FIELD_FLOOR_TEXTURES, FIELD_MAP } from "./chapter1Data";
...
// 書き出すテキストで使う文字。chapter1Data.tsのFLOOR_CHAR_MAPと揃えてある
...
const FLOOR_ORDER: FloorTileType[] = ["grass", "dirt", "water", "sand", "stone"];
...
const OBJECT_PALETTE: ObjectDef[] = [
  ...
  { id: "tower", label: "塔", image: `${TILESET_DIR}/icon_tower_a_body.png`, widthTiles: 2, heightTiles: 4, blocksMovement: true },
  { id: "crate", label: "木箱", image: `${TILESET_DIR}/icon_crate_a.png`, widthTiles: 1, heightTiles: 1, blocksMovement: true },
  { id: "rubble", label: "瓦礫", image: `${TILESET_DIR}/icon_wall_piece.png`, widthTiles: 1, heightTiles: 1, groundLevel: true },
  { id: "hill", label: "丘", image: `${TILESET_DIR}/icon_hill_orange.png`, widthTiles: 1, heightTiles: 1, blocksMovement: true },
];
...
export function MapEditor() {
  ...
  const loadExisting = (which: "town" | "field") => {
    ...
    setObjects([]); // 参考表示は床・壁だけ。置物・会話ポイントまでは読み込まない（まっさらな状態で見せる）
    setMarkers([]);
    setWidth(w);
    setHeight(h);
    setWidthInput(String(w));
...
// --- 床の見た目 ---（chapter1Data.tsのFLOOR_CHAR_MAPと同じ文字: . 草地 / # 土の道 / ~ 水面 / s 砂地 / o 岩地）
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white p-4 md:p-6">
      <div className="max-w-6xl mx-auto space-y-4">
        ...
        {mode === "paint" && (
          <div className="bg-slate-800 rounded-lg p-3 flex flex-wrap items-center gap-2 text-sm">
            <span className="text-slate-300 mr-1">床の筆:</span>
            {FLOOR_ORDER.map((floor) => (
              <button
                key={floor}
                onClick={() => setBrush({ kind: "floor", floor })}
                className="rounded px-3 py-1.5 flex items-center gap-2 border-2"
                style={{
                  backgroundColor: FLOOR_COLORS[floor],
